...
app.get("/getTweetList/", function(req, res) {
	...
	if (parseInt(req.query.fromId) !== 0) {
		url += "&max_id=" + req.query.fromId;
	}
	console.log(url);
	oauth.get(
		url,
		req.headers["clienttoken"],
		req.headers["clientsecrettoken"],
	...
	request.post(
		{
			...
			oauth: {
				oauth_callback: "http://localhost:3000/close",
...
router.route("/auth/twitter").post(function(req, res, next) {
	request.post(
		{
